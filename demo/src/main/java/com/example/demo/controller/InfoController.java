package com.example.demo.controller;

import org.springframework.stereotype.Controller;
import org.springframework.ui.ModelMap;
import org.springframework.web.bind.annotation.GetMapping;

import jakarta.servlet.http.HttpSession;


@Controller
public class InfoController {
 @GetMapping("/info")
    public String info(ModelMap m, HttpSession s) {
        String mensaje = (s.getAttribute("_mensaje") != null) 
                ? (String) s.getAttribute("_mensaje") 
                : "Pulsa el botón para volver";
        
        String link = (s.getAttribute("_link") != null) 
                ? (String) s.getAttribute("_link") 
                : "Pulsa el botón para volver"; 
        String severity = (String) s.getAttribute("_severity");

        m.put("mensaje", mensaje);
        m.put("link", link);
        m.put("severity", severity);
        
        s.removeAttribute("_mensaje");
        s.removeAttribute("_link");
        s.removeAttribute("_severity");

        m.put("view", "_t/info"); 
        
        return "_t/frame";
    }
    

}
